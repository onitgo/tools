...
function constructKeyboard(bool) {
	...
	if(typeof(bool) == 'undefined')
		check = document.getElementById('multilingualKeyBoard').checked;
	else
		check = bool;
	
	var keyArr = getArr();
	var str_trco="</tr><tr>";
	sHTML="<table	border='0' class='keyboardtb_new' cellspacing='3px' id='keypad' width='100%' align='center'><tr>";
	var dHTML="";
		...
		for (var j=0; j<keyArr[i].length; j++){
			...
			if(typeof(keyArr[i][j])=='object'){
				...
				if(check)
					...
					sHTML="<table	border='0' class='keyboardtbl' cellspacing='3px' id='keypad' width='100%'><tr><td></td></tr></table>";
					//dHTML="<span id='logimg'></span>";
			}
		}
		sHTML+=str_trco;
	}
	if(check){
	sHTML+="<td colspan='5' id='cap' onClick='setCaps(this)' class='keyboardtblenb' style='padding:5px 7px; width:100px; font-size:85%;'>CAPS LOCK</td>";
	sHTML+="<td colspan='5' id='clr' onClick='setClearAll()' class='keyboardtblenb' style='padding:5px 7px; width:100px; font-size:85%;'>CLEAR</td>";}
	else{
		sHTML="<table	border='0' class='keyboardtbl' cellspacing='3px' id='keypad' width='100%'><tr><td></td></tr></table>";
	}
	...
	document.querySelector('#kbplaceholder').innerHTML = sHTML;
}
shuffle = function(v){
